...
const contextTiers = [8192, 16384, 24576, 32768, 49152, 65536]
...
function taskCost(task) { return task.contextBudget + task.outputReserveTokens + task.safetyReserveTokens + args.safetyReserveTokens }
function workerPrompt(task, reports) {
  const packet = { ...task.taskPackage, relevantContext: [...task.taskPackage.relevantContext, ...reports.filter((report) => task.dependsOn.includes(report.taskId)).map((report) => JSON.stringify(report))], constraints: [...task.taskPackage.constraints, args.allowWrites ? 'Writes are allowed only inside declared writeScopes.' : 'Read-only: do not edit files.'] }
  return ['You are one isolated ' + task.role + ' worker.', 'Do not start another orchestration.', 'TASK:\\n' + task.title + '\\n' + task.prompt, 'GOAL:\\n' + packet.goal, 'RELEVANT CONTEXT:\\n' + JSON.stringify(packet.relevantContext), 'CONSTRAINTS:\\n' + JSON.stringify(packet.constraints), 'KNOWN FACTS:\\n' + JSON.stringify(packet.knownFacts), 'FILES / CODE:\\n' + JSON.stringify(packet.files), 'DEPENDENCIES:\\n' + JSON.stringify(packet.dependencies), 'EXPECTED OUTPUT:\\n' + packet.expectedOutput, 'DO NOT:\\n' + JSON.stringify(packet.doNot), 'Declared write scopes:\\n' + JSON.stringify(task.writeScopes), 'Return a strict structured report. If the packet is insufficient, return status needs_more_context and a NEED_MORE_CONTEXT item instead of guessing.'].join('\\n\\n')
}
function nextTier(current, requested) {
  if (requested !== undefined && requested > current) return requested
  return contextTiers.find((tier) => tier > current && tier <= 65536)
}
...
while (remaining.length > 0) {
  ...
  for (const candidate of candidates) {
    const conflicts = batch.some((selected) => scopesOverlap(selected.writeScopes, candidate.writeScopes))
    const batchHasWrite = batch.some(hasWrite)
    const canShareBatch = (!hasWrite(candidate) && !batchHasWrite) || (args.allowParallelWrites && !conflicts)
    if (batch.length < Math.min(args.maxConcurrentAgents, args.maxWorkers) && canShareBatch && used + taskCost(candidate) <= args.totalContextTokens) { batch.push(candidate); used += taskCost(candidate) }
  }
  if (batch.length === 0) { const task = ready[0]; remaining.splice(remaining.indexOf(task), 1); states.set(task.id, 'FAILED'); workers.push({ taskId: task.id, status: 'blocked', summary: 'Task exceeds the available batch context budget.', evidence: [], changedFiles: [], tests: [], blockers: ['Reduce the requested context tier or increase totalContextTokens.'], nextSteps: [] }); continue }
  for (const task of batch) { const position = remaining.indexOf(task); if (position >= 0) remaining.splice(position, 1); states.set(task.id, 'ACTIVE'); attempts.set(task.id, (attempts.get(task.id) || 0) + 1) }
  ...
  for (let index = 0; index < batch.length; index += 1) {
    ...
    if (escalated && higher !== undefined && (attempts.get(task.id) || 0) < 3) { task.contextBudget = higher; task.taskPackage = { ...task.taskPackage, relevantContext: [...task.taskPackage.relevantContext, 'Previous worker attempt:\\n' + JSON.stringify(report)] }; remaining.push(task); states.set(task.id, 'WAITING'); continue }
    workers.push(report)
